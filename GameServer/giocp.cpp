...
BOOL RecvDataParse(LPPER_IO_CONTEXT	lpIOContext, int uIndex)	
{
	...
	while ( 1 )
	{
		...
		if ( size <= lpIOContext->nSentBytes )
		{
			if ( xcode == 0xC3 )
			{
				//int iSize = g_CryptoSessionCS.Decrypt( byDec+2,(recvbuf+lOfs+2), size-2);
				int iSize = g_CryptoSessionCS.Decrypt( uIndex, byDec+2, (recvbuf+lOfs+2), size - 2);

				if ( iSize < 0 )
				{
					LogAddC(2,"Decrypt1> Failed %d",uIndex); //MyFix 3
				}
				else
				{
			...
			else if ( xcode == 0xC4 )
			{
				...
				int iSize = g_CryptoSessionCS.Decrypt( uIndex, byDec+3, (recvbuf+lOfs+3), size-3);
                if ( iSize < 0 )
				{
					LogAddC(2,"Decrypt2> Failed %d",uIndex); //MyFix 3
				}
				else
				{
...
int DataSend(int aIndex, LPBYTE lpMsg, DWORD dwSize)
{
	...
	if ( lpMsg[0] == 0xC3 || lpMsg[0] == 0xC4 )
	{
		...
		if ( lpMsg[0] == 0xC3 )
		{
			...
			//ret = g_CryptoSessionSC.Encrypt(aIndex,&ExSendBuf[2], &lpMsg[1], dwSize-1);
			ret = g_CryptoSessionSC.Encrypt(aIndex, ExSendBuf + 2, lpMsg+1, dwSize-1);
			ExSendBuf[0] = 0xC3;
			ExSendBuf[1] = ret + 2;
			SendBuf = ExSendBuf;
		...
		else
		{
			...
			ret = g_CryptoSessionSC.Encrypt(aIndex,&ExSendBuf[3], &lpMsg[2], dwSize-2);
			ExSendBuf[0] = 0xC4;
